...
namespace SYN {
    ...
    enum TicketType
    {
        ...
        TechnicalIssues,
        AccountAccessIssues,
        BlacklistORBanAppeal, 
        BugReport,
        EmailChangeRequest,
        Unknown
    }
    export function ParseStringIntoTicketType(s :string) 
    {
     ...
     */
    export function ParseTicketTypeIntoString(TT :TicketType) 
    {
    ...
    {
        [key:string] : any
        public UncapTickets : boolean;
        public CustomFixes  : Fix[]
...
function GetDataFromAgentDocument(doc:Document, modify :boolean) 
{
    ...
    let responses : SYN.Response[] = [] // pre initialize the arr
    for (let boxi in boxes) // loop over all the boxes
    {
        ...
        responses = [ // updates t he array
            ...responses, // use spread the old arr
            new SYN.Response( // add a new response
            {
                Text:text.text(),
                Time: new Date((time.text() as string).slice(0,-2) + 'GMT-0400'),
                User: replier.text(),
                isAgent: isAgent
            })
        ]
        ...
        time.replaceWith(`<h5 class="subtitle is-5">At <span style="font-weight:bold">${new Date((time.text() as string).slice(0,-2) + 'GMT-0400').toLocaleString(undefined,{'hour12':true})}</span></h5>`) // changes what the time looks like
    ...
    return new SYN.Agent({ // return the agent
        Agent: 0 in filter ? filter[0].User : 'N/A',
        Messages:responses,
        Id:id.innerText,
        Opened: new Date((opened.innerText as string).slice(0,-2) + 'GMT-0400'),
...
const getDataFromBox = (box : Element) =>
{
    ...
    let Opened = new Date((jquery.children(':nth-child(3)').children(':last').text() as string).slice(0,-2) + 'GMT-0400');      // parses the time 
    let LastUpdated = new Date((jquery.children(':nth-child(4)').children(':last').text()as string).slice(0,-2) + 'GMT-0400'); // to a local format
    return new SYN.Ticket({ // return the ticket data 
        Id : id,
        User : User,
...
function GetTicketsFromDocument(doc : Document, Modify:boolean) {

    let [NonOwner,Owner] = $(doc).find('.columns.is-mobile.is-multiline:not([id="C"])').get()
    if (!Owner || !NonOwner) return {'Error':33,'Message':'Expected to have a /tickets document'}
    ...
    let claimed : SYN.Ticket[] = [];
    let OtherBoxes : SYN.Ticket[] = [];
    for (let key in Combined) {
        let v = Combined[key]
        if (v.nodeName != 'DIV') continue
        let box = getDataFromBox(v.firstElementChild as Element)
        if (box.Agent == GetCurrentAgent()) claimed = [...claimed,box]
        else OtherBoxes = [...OtherBoxes,box]
...
function GetArrayOfMissingValues(ch : any[], comp : any[]) 
{
...
function GenerateTicketFromData(data:SYN.Ticket) 
{
...
function TICKET_MAIN() // ran on tickets page
{
    ...
    })
    {
        ...
        if (tickets.Error) console.error(resp.Message)        //handle error
        if (!tickets) window.location.reload()  //caused by getting signed out
        ...
        for (let ticketC of ticketsC.new as SYN.Ticket[]) {

            GenerateTicketFromData(ticketC).appendTo('#C')

        }
        for (let ticket of ticketsv.new as SYN.Ticket[]) {
            
            GenerateTicketFromData(ticket).appendTo(`${ticket.IsOwner ? '#O' : '#NO'}`)
            
        }
        for (let ticketC of ticketsC.missing as SYN.Ticket[]) {

            $('#C').find(`[id=${ticketC.Id}]`).remove()

        }
        for (let ticket of ticketsv.missing as SYN.Ticket[]) {
    ...
    //settings 
    let Settings = GetData('Settings') as SYN.Settings ?? new SYN.Settings({UncapTickets:false,CustomFixes:[],AutoRefresh:false})
    ...
    Setb.click(()=>{
        ...
        if (SettingsUIT) {
            ...
            let deleteF = (ele : any)=>{
                ...
                let name = $(ele.target).parent().find('input[class="input is-small Name"]').val() as string
            ...
            newF.find('input[class="button add"]').css({'display':'inline',"height":"auto"}).click((ele)=>{
                ...
                let name = n.find('input[class="input is-small Name"]').val() as string
                let value = n.find('textarea[class="input is-small Value"]').val() as string
    ...
    if (resp.Error) throw new Error(resp.Message)       //handle 
    if (!resp) throw new Error('Something went wrong')// errors
...
function AGENT_MAIN() // ran on agent pages
{ 
    let reply = GetDataFromAgentDocument(document,true) // get data from the agent page
    let Settings = GetData('Settings') as SYN.Settings ?? new SYN.Settings({UncapTickets:false,CustomFixes:[],AutoRefresh:false})
    //console.log(reply)

    function Confirmation(type:String) { // function to be passed into a script tag to confirm that the user meant to solve / ban
    ...
    function insertInto(ele : any) {

        ele = ele.target as Element;
        let name = $(ele).val()
...
$(document).ready(()=>{ 
    ...
    switch (WebsiteType) 
    {
        ...
        default         : {console.log(window.location.href);console.log('Wasn\'t able to match this page, contact pozm with a screenshot of this whole console.')} return;
    }
})